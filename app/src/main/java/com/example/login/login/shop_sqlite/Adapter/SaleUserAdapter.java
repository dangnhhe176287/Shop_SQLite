...
public class SaleUserAdapter extends ArrayAdapter<SaleUserResponseDto> {

    private final List<SaleUserResponseDto> users;
    private final OnUserActionListener listener;

    // Interface để gửi sự kiện click ra ngoài Activity (ĐÃ CẬP NHẬT)
    public interface OnUserActionListener {
        void onEditClick(SaleUserResponseDto user);
        void onDeleteClick(SaleUserResponseDto user);
        void onDetailClick(SaleUserResponseDto user); // <--- THÊM PHƯƠNG THỨC NÀY
    }

    public SaleUserAdapter(Context context, List<SaleUserResponseDto> users, OnUserActionListener listener) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        SaleUserResponseDto currentUser = getItem(position);
