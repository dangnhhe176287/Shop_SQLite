...
import com.example.login.login.shop_sqlite.Dto.SaleUserResponseDto; // Ensure this DTO exists
...
public class SaleUserDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.sale_activity_user_detail);
    ...
    private void fetchUserDetails(int id) {
        ...
        saleApiService.getUserById(id).enqueue(new Callback<SaleUserResponseDto>() {
            @Override
            public void onResponse(Call<SaleUserResponseDto> call, Response<SaleUserResponseDto> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    Toast.makeText(SaleUserDetailActivity.this, errorMsg, Toast.LENGTH_LONG).show();
            ...
            public void onFailure(Call<SaleUserResponseDto> call, Throwable t) {
                ...
                Toast.makeText(SaleUserDetailActivity.this, "Lỗi kết nối server khi tải chi tiết người dùng.", Toast.LENGTH_LONG).show();
                finish(); // Close activity if connection error
            }
        });
    }

    private void displayUserDetails(SaleUserResponseDto user) {
